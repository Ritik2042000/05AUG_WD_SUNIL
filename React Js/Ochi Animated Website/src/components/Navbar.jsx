...
const Navbar = () => {
    ...
    return (
        <motion.div className='fixed z-[999] w-full px-20 py-8 flex items-center  justify-between font-["NeueMontreal"] bg-[rgba(255,255,255,0.02)] backdrop-blur-md'
            initial={{ y: 0 }}
            animate={{ y: showNavbar ? 0 : -100 }}  // Animate to -100 to hide it
        ...
        >
            ...
            <div className="links flex gap-10">
                {['Services', 'Our Work', 'About Us', 'Insights', 'Contact Us'].map((items, index) => {
                    return <a href="#" key={index} className={`text-lg capitalize font-light ${index === 4 && 'ml-32'}`}>{items}</a>
                })}
            </div>
        </motion.div>
    );
