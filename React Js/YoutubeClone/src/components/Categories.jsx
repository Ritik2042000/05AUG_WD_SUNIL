        ...
        <div className='w-[100%] overflow-x-scroll flex scrollbar-hide my-2 '>
            {
                buttonsOfCategories.map((data) => {
                    return <div className='' key={data.id}>
                        <buttons className={`${btnactive == data.title ? 'bg-slate-900 text-white' : 'bg-gray-200 hover:bg-gray-300'} px-3  py-1 cursor-pointer rounded-xl mx-2 text-sm  inline-block w-max `} onClick={() => selectCategories(data.title)}>
                            {data.title}
                        </buttons>
                    </div>
                })
