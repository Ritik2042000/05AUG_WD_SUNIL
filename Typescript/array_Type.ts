...
const arryOfNames:string[] = ['xyzhgcc','qwfdffdssdsdade','rohanandmohan','soham','mohan','anil']

const doubleData:number[] = arryOfNumbers.map((data:number) => data*2)
console.log(doubleData);

const numberToString:string[] = arryOfNumbers.map((data:number)=>data.toString())
console.log(numberToString);

const filteredData:number[] = arryOfNumbers.filter((data:number) => data > 3 );
console.log(filteredData);

const squreOfNumbers:number[] = arryOfNumbers.map((data:number) => data * data)
console.log(squreOfNumbers);

const capitalNames:string[] = arryOfNames.map((data:string) => data.toUpperCase())
console.log(capitalNames);

const wordWith15length:string[] = arryOfNames.filter((data:string) => data.length > 5)
console.log(wordWith15length);

const fiterNameWithA = arryOfNames.filter((data:string) => data[0] === 'a')
const fiterNameWithA_method2 = arryOfNames.filter((data:string) => data.startsWith('a'))
